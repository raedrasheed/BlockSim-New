...
## TV14 — RESERVE activation passes through WAKING and charges wake energy
`ReserveActivate` (miner RESERVE): select reserve; **T4** RESERVE→WAKING → `WakeComplete`: `[SIMULATION SAMPLING] wake_latency_model`; ACCUMULATE t_wake at P_wake + E_transition; validate; activate PENDING→CURRENT; **T5** →ACTIVE_HASHING. Wake energy explicitly charged. Invariants: D2, I6, I10 (no overlap).
