"""Stage 8Y — protected-artifact SHA-256 baseline.

    python -m experiments.stage8y.baseline --record     # create the baseline
    python -m experiments.stage8y.baseline --check      # verify nothing changed

Stage 8Y must not modify Stage 8X, any earlier experiment, any thesis document,
any legacy model, test or result. This module takes a SHA-256 fingerprint of every
such artifact, stores it under ``experiments/stage8y/manifests/``, and re-verifies
it after execution.

Anything under ``experiments/stage8y/`` is deliberately excluded: that is the only
tree Stage 8Y is permitted to write to.
"""
...
BASELINE_FILE = os.path.join(MANIFEST_DIR, "stage8y_protected_baseline.json")

#: Everything Stage 8Y must leave byte-identical.
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="Stage 8Y protected-artifact baseline")
