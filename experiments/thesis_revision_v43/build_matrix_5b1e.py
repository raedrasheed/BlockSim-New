"""Stage 5B1E: regenerate the matrix after the inactive-range / exact-accounting /
lifecycle corrections. Scientific semantics are unchanged; the engine version is
bumped to 5b1e.1, so scientific_semantics_hash and run_execution_hash change.
...
RAW = os.path.join(ROOT, "results", "thesis_revision_v43", "stage_05b1e", "raw")
...
def write(final, stats, retention):
    ...
    matrix = os.path.join(DOCS, "STAGE_05B1E_FINAL_MATRIX.csv")
    ...
    summary = dict(
        stage="5B1E", scenario_engine_version=ENGINE_VERSION,
    ...
    json.dump(summary, open(os.path.join(RAW, "matrix_5b1e_summary.json"), "w"), indent=2)
    json.dump(aud, open(os.path.join(DOCS, "STAGE_05B1E_HASH_GROUP_AUDIT.json"), "w"), indent=2)
