"""Stage 5B1D internal freeze-INPUT manifest (for freeze #2).

Created BEFORE the corrective commit. Records the frozen inputs and the INTENDED
freeze branch name (thesis-v43-stage5b2-freeze-2) but makes NO claim to contain the
...
commit exists. The old freeze (thesis-v43-stage5b2-freeze-1) is preserved unchanged
...
INTENDED_FREEZE_BRANCH = "thesis-v43-stage5b2-freeze-2"
SUPERSEDES = "thesis-v43-stage5b2-freeze-1"
...
def build_freeze_inputs(test_result: str = "unverified") -> dict:
    ...
    sp = os.path.join(RESULTS, "raw", "matrix_5b1d_summary.json")
