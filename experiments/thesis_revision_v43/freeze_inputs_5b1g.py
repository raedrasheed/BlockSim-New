"""Stage 5B1G internal freeze-INPUT manifest (for freeze #5).

Created BEFORE the corrective commit. Records the frozen inputs and the INTENDED
freeze branch name (thesis-v43-stage5b2-freeze-5) but makes NO claim to contain the
SHA of the commit that will contain it (contains_own_commit_sha: false). The
authoritative commit SHA is recorded externally (in the completion report and the
new remote freeze branch) after the commit exists. All earlier freeze branches
(freeze-1 .. freeze-4) are PRESERVED unchanged as historical invalidated freezes;
freeze-4 is explicitly marked invalid for execution.
"""
...
INTENDED_FREEZE_BRANCH = "thesis-v43-stage5b2-freeze-5"
SUPERSEDES = "thesis-v43-stage5b2-freeze-4"
PRESERVED_FREEZES = ("thesis-v43-stage5b2-freeze-1", "thesis-v43-stage5b2-freeze-2",
                     "thesis-v43-stage5b2-freeze-3", "thesis-v43-stage5b2-freeze-4")
...
def build_freeze_inputs(test_result: str = "unverified") -> dict:
    matrix = os.path.join(DOCS, "STAGE_05B1G_FINAL_MATRIX.csv")
    sp = os.path.join(RESULTS, "raw", "matrix_5b1g_summary.json")
    ...
    return dict(
        ...
        superseded_freeze_invalid_for_execution=SUPERSEDES,
        preserved_freezes=list(PRESERVED_FREEZES),
