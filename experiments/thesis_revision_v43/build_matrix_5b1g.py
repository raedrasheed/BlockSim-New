"""Stage 5B1G: regenerate the matrix after the stale-race diagnostic isolation,
delay-stream correction, solution-taxonomy correction and exact B2 exhaustion
closure. Scientific scenario semantics are UNCHANGED (63 groups x 30 seeds = 1 890
runs); the engine version is bumped to 5b1g.1, so scientific_semantics_hash and
run_execution_hash change. Preserves every earlier matrix. Does NOT execute the
matrix.
"""
...
RAW = os.path.join(ROOT, "results", "thesis_revision_v43", "stage_05b1g", "raw")
...
def write(final, stats, retention):
    ...
    summary = dict(
        stage="5B1G", scenario_engine_version=ENGINE_VERSION,
    ...
    json.dump(summary, open(os.path.join(RAW, "matrix_5b1g_summary.json"), "w"), indent=2)
    json.dump(aud, open(os.path.join(DOCS, "STAGE_05B1G_HASH_GROUP_AUDIT.json"), "w"), indent=2)
