"""Stage 8Z — protected-artifact SHA-256 baseline.

    python -m experiments.stage8z.baseline --record     # create the baseline
    python -m experiments.stage8z.baseline --check      # verify nothing changed

Stage 8Z must not modify Stage 8X, Stage 8Y, any earlier experiment, any thesis document,
any legacy model, test or result. This module takes a SHA-256 fingerprint of every
such artifact, stores it under ``experiments/stage8z/manifests/``, and re-verifies
it after execution.

Anything under ``experiments/stage8z/`` is deliberately excluded: that is the only
tree Stage 8Z is permitted to write to.
"""
...
BASELINE_FILE = os.path.join(MANIFEST_DIR, "stage8z_protected_baseline.json")

#: Everything Stage 8Z must leave byte-identical.
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="Stage 8Z protected-artifact baseline")
