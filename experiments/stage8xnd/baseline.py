"""Stage 8X-ND — protected-artifact SHA-256 baseline.

    python -m experiments.stage8xnd.baseline --record     # create the baseline
    python -m experiments.stage8xnd.baseline --check      # verify nothing changed

Stage 8X-ND must not modify Stage 8X, Stage 8Y, Stage 8Z, any earlier experiment, any thesis document,
any legacy model, test or result. This module takes a SHA-256 fingerprint of every
such artifact, stores it under ``experiments/stage8xnd/manifests/``, and re-verifies
it after execution.

Anything under ``experiments/stage8xnd/`` is deliberately excluded: that is the only
tree Stage 8X-ND is permitted to write to.
"""
...
BASELINE_FILE = os.path.join(MANIFEST_DIR, "stage8xnd_protected_baseline.json")

#: Everything Stage 8X-ND must leave byte-identical.
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="Stage 8X-ND protected-artifact baseline")
