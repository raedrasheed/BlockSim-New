...
from _common import (ARCHIVE, RUN_LEVEL, Registry, TERMINAL_SUCCESS,           # noqa: E402
                     frozen_rows, _atomic_write)

IDENTITY = ["run_id", "scenario_id", "block_id", "master_seed", "seed_index", "pair_id",
            "paired_control_id", "cost_class", "run_status", "config_sha256",
            "engine_commit_sha", "attempt", "wall_clock_seconds", "result_sha256",
            "compressed_sha256", "exception_type", "exception_detail"]
...
def build() -> tuple:
    ...
    reg = {r["run_id"]: r for r in Registry().read()}
    rows = []
    for fr in frozen_rows():
        rid = fr["run_id"]
        st = reg.get(rid, {})
        srow = srows[fr["scenario_id"]]
        row = {k: "" for k in IDENTITY + outcomes + INTEGRITY + ["zero_block_indicator"]}
        ...
        for k in ("config_sha256", "engine_commit_sha", "attempt", "wall_clock_seconds",
                  "result_sha256", "compressed_sha256", "exception_type", "exception_detail"):
            row[k] = st.get(k, "")
        if st.get("run_status") in TERMINAL_SUCCESS:
            ...
            for name in outcomes + INTEGRITY + ["zero_block_indicator"]:
                row[name] = "NA"                              # not executed: NA, not zero
        rows.append(row)
    return rows, IDENTITY + ["zero_block_indicator"] + outcomes + INTEGRITY
...
def main(argv=None) -> int:
    ...
    for r in rows:
        w.writerow(r)
    dest = pathlib.Path(a.out) if a.out else (RUN_LEVEL / "stage07_run_level.csv")
